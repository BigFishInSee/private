        ...
        for index, option in enumerate(obs.select.option):
            match option.type:
                ...
                case OptionType.ABILITY:
                    if u.id_from_option(option, obs) == CARDS.DUSKNOIR and opponent.active[0].hp <= 130:
                        return True, [index]
                    elif u.id_from_option(option, obs) == CARDS.DUSCLOPS and opponent.active[0].hp <= 50:
                        return True, [index]
                case _:
                    pass
    #CHECK BENCH
        ...
        if u.prize_card_drop(Pokemon.id) >= prizeTarget:
            ...
            for index, option in enumerate(obs.select.option):
                match option.type:
                    ...
                    #WIN WITH A DUSKNOIR ABILITY 
                    case OptionType.ABILITY:
                        if u.id_from_option(option, obs) == CARDS.DUSKNOIR and Pokemon.hp <= 130:
                            return True, [index]
                        elif u.id_from_option(option, obs) == CARDS.DUSCLOPS and Pokemon.hp <= 50:
                            return True, [index] 
                    case _:
                        pass
    return False, [999]
