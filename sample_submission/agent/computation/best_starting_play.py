...
def follow_up_bench(hand: list[Card], legal: list[Option], obs: Observation):
    ...
    if (has_card_count(hand, CARDS.PSYCHIC_ENERGY)+has_card_count(hand, CARDS.TELEPATHIC_ENERGY)>=2) and (has_card_count(hand, CARDS.SECRET_BOX)+has_card_count(hand, CARDS.LILLIE_DETERMINATION)+has_card_count(hand, CARDS.MYSTERY_GARDEN)+has_card_count(hand, CARDS.ULTRA_BALL)+has_card_count(hand, CARDS.POKE_PAD)>3):
        pokemon_plan[CARDS.MEOWTH].Start_Bench_priority -= 1
    #find the active
    for count, log in enumerate(obs.logs):
        if log.type == 6 and log.toArea == 4:
            #found pokemon last activated
            pokemon_plan[log.cardId].Start_Bench_priority -= 1
